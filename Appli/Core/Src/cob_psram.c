...
static HAL_StatusTypeDef COB_PSRAM_Command(uint32_t address, uint32_t size)
{
  ...
  command.AddressSpace = HAL_XSPI_MEMORY_ADDRESS_SPACE;
...
HAL_StatusTypeDef COB_PSRAM_Write(uint32_t address, const uint8_t *data, uint32_t size)
{
  ...
  status = COB_PSRAM_Command(address, size);
  if (status == HAL_OK)
  {
